Below is a precise, self‑contained description of the project you’re about to work with.
You can treat it as an orientation briefing for a peer LLM that will read the codebase and act as an agent or co‑developer.
...
The 2 KB guideline isn’t a hard ceiling; it’s a “sweet‑spot” heuristic.
Here’s why we still recommend lean Layer‑0 contexts even though token windows are exploding:

Reason to keep Layer 0 small	Applies even with 1 M‑token windows?	Details
Attention focus	✔	Longer prompts dilute relevant signal; most chain‑of‑thought planners work better if the first few kilobytes contain only what’s needed.
Latency & cost	✔	A 1 M window inference is still slower and more expensive than a 2 k window one—especially in interactive loops where the tool may be called dozens of times per turn.
Cache‑friendliness	✔	Composer’s LRU cache, browser caches, and CDN edge caches all work best on tiny JSON documents; fat contexts get fetched and parsed more often than you think.
Hash‑stable diffing	✔	When you version contexts by content‑hash, changing a single comma in a 50 KB file busts the hash and invalidates caches. Tiny contexts localise churn.
Prompt‐engineering debugging	✔	If the context snippet is a screenful, you can eyeball it; 20 KB of prefix mappings is unreadable in logs/chat transcripts.

Where big windows do matter
	•	Layer 1/2 Artefacts (ontology.ttl, shapes.ttl, rules.ttl) can be
much larger—tens or hundreds of KB—because the agent doesn’t keep them
in its main prompt.  It streams them into the sandbox via the tool
call; the model only needs to reference their IRIs or short
affordance‑hints in its context.
	•	Layer 3 Data can grow: a million‑token model can ingest several
full documents at once.  But you still get better thinking quality
if you focus the window (e.g. by time‑slice or by entity).

Practical rule‑of‑thumb

Layer	Recommended size if you have…	≤ 32 K window	≥ 1 M window
0 Context	1–2 KB	1–5 KB	
1 Ontology	20 KB	200 KB (entire OWL)	
2 Shapes/Rules	30 KB	300 KB (multiple rule bundles)	
3 Data per call	20 KB	500 KB–1 MB	

(Nothing breaks if you exceed those—these are comfort zones.)

How to exploit huge windows without bloating Layer 0
	1.	Chunking by need
Let the LLM decide which shapes or ontology fragments to pull into
its scratch‑pad.  It can issue a reason_over(query=...) call that
selects only the shapes relevant to the task.
	2.	Context‑on‑demand
Provide secondary contexts (e.g. a big term‑definition file) that the
agent can copy snippets from when it realises it needs more
vocabulary, instead of front‑loading everything.
	3.	Iterative fetch
With a massive window you can loop: ask, fetch more, refine,
stopping when answer quality saturates.

Bottom line

Large context windows free you from frugal token budgets, but focus is
still critical for reasoning quality, latency, interpretability, and
caching.  Keeping the mandatory @context small preserves those
advantages while letting higher layers scale as the model’s capacity
grows.
